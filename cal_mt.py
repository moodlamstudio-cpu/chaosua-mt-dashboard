...
# col: 0=Month,1=MakroAct,2=MakroLE,3=LotusAct,4=LotusLE,5=MakroLY,6=LotusLY,7=MakroAOP,8=LotusAOP
...
le_mix=(sum(mon[m]["act"] for m in range(1,8))+mon[8]["le"])/1e6   # Actual M1-7 + LE M8
ly=sum(mon[m]["ly"] for m in range(1,9))/1e6
aop=sum(mon[m]["aop"] for m in range(1,9))/1e6
print(f"MT: Actual={act:.2f} LE(mix)={le_mix:.2f} LY={ly:.2f} AOP={aop:.2f} vsAOP={le_mix/aop*100:.1f}%")
for m in [4,8]:
    print(f"  M{m}: actual={mon[m]['act']/1e6:.2f} le={mon[m]['le']/1e6:.2f} ly={mon[m]['ly']/1e6:.2f} aop={mon[m]['aop']/1e6:.2f}")
wb.close()
