...
function drawSku(c){var total=rangeSum(actSeries(c),selFrom,selTo);var list;
 ...
 var shown=list.slice(),ta=0,tl=0,allLy=true;if(activeSku){var sf=c.monthly_by_sku&&c.monthly_by_sku[activeSku];if(sf)total=rangeSum(sf.actual,selFrom,selTo);}else if(activeCat){var cf=c.monthly_by_cat&&c.monthly_by_cat[activeCat];if(cf)total=rangeSum(cf.actual,selFrom,selTo);}shown.sort(function(a,b){return perfValues(c.monthly_by_sku&&c.monthly_by_sku[b.name]).actual-perfValues(c.monthly_by_sku&&c.monthly_by_sku[a.name]).actual;});shown.forEach(function(s,i){var cat=s.cat||activeCat||"",v=perfValues(c.monthly_by_sku&&c.monthly_by_sku[s.name]),p=total?v.actual/total*100:0,cls='skuro perf-row'+(v.growth!=null&&v.growth>0?' growing':'')+(activeSku===s.name?' selected':'');ta+=v.actual;if(v.ly==null)allLy=false;else tl+=v.ly;h+='<tr class="'+cls+'" data-i="'+i+'"><td>'+(i+1)+'</td><td style="max-width:260px;white-space:normal;text-align:left">'+s.name+'</td><td>'+cat+'</td><td>'+perfCell(v.ly)+'</td><td>'+fmt(v.actual)+'</td><td>'+signedPerf(v.growth)+'</td><td>'+signedPerf(v.pct,'%')+'</td><td>'+fmt(p)+'%</td></tr>';});
 var tg=allLy?ta-tl:null,tp=(allLy&&tl!==0)?tg/tl*100:null;
 h+='</tbody><tfoot><tr class="total"><td></td><td style="text-align:left">Total</td><td></td><td>'+perfCell(allLy?tl:null)+'</td><td>'+fmt(ta)+'</td><td>'+signedPerf(tg)+'</td><td>'+signedPerf(tp,'%')+'</td><td>100.0%</td></tr></tfoot>';setT("skuTable",h);
