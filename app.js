...
var ANNUAL_CH_COLORS=["#111111","#525252","#8a8a8a","#c7c7c7","#e5e5e5"];
function annualFmt(v){return Math.round(num(v)).toLocaleString("en-US");}
function annualPct(v){return Math.round(num(v))+"%";}
function annualChannelValues(ch,focusName,focusIsSku,y,month){
  var src=D&&D[ch]||{},hist;
  if(focusName){var map=focusIsSku?src.history_by_sku:src.history_by_cat;hist=map&&map[focusName]&&map[focusName][String(y)]||{};}
  else hist=src.history&&src.history[String(y)]||{};
  if(month)return num(hist[String(month)]);
  var total=0;for(var m=selFrom;m<=selTo;m++)total+=num(hist[String(m)]);return total;
}
var annualLabelPlugin={id:"annualLabelPlugin",afterDatasetsDraw:function(chart){var ctx=chart.ctx;ctx.save();chart.data.datasets.forEach(function(ds,di){var meta=chart.getDatasetMeta(di);if(!meta||!meta.data)return;meta.data.forEach(function(bar,i){var v=num(ds.data[i]);if(!v||!bar)return;var total=0;chart.data.datasets.forEach(function(x){total+=num(x.data[i]);});var con=total?v/total*100:0,x=bar.x,y=(bar.y+bar.base)/2;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillStyle=di<3?"#ffffff":"#111111";ctx.font="700 10px Arial,sans-serif";if(Math.abs(bar.base-bar.y)>=30){ctx.fillText(ds.label,x,y-6);ctx.font="600 9px Arial,sans-serif";ctx.fillText(annualFmt(v)+" MB · "+annualPct(con),x,y+7);}else if(Math.abs(bar.base-bar.y)>=16){ctx.font="600 8px Arial,sans-serif";ctx.fillText(annualFmt(v)+" MB",x,y);}});});chart.data.labels.forEach(function(_,i){var total=0,top=null;chart.data.datasets.forEach(function(ds,di){total+=num(ds.data[i]);var meta=chart.getDatasetMeta(di);if(num(ds.data[i])&&meta&&meta.data&&meta.data[i])top=meta.data[i];});if(!top||!total)return;ctx.textAlign="center";ctx.textBaseline="bottom";ctx.fillStyle="#475569";ctx.font="700 12px Arial,sans-serif";ctx.fillText(annualFmt(total),top.x,top.y-7);});ctx.restore();}};
function drawAnnual(){destroy("cAnnual");var c=C(),focus=currentFocus(c),focusName=focus&&focus.name||null,focusIsSku=!!activeSku,month=kpiPick&&(kpiPick.type==="month"?+kpiPick.value:(kpiPick.type==="yearMonth"?+kpiPick.month:null)),rangeLabel=MONTHS[selFrom-1]+"\u2013"+MONTHS[selTo-1],ids=(selChans&&selChans.length?selChans:[cur]),years=[],ys={};ids.forEach(function(ch){var src=D&&D[ch]||{};Object.keys(src.history||{}).forEach(function(y){ys[y]=1;});});years=Object.keys(ys).sort();var datasets=ids.map(function(ch,di){var src=D&&D[ch]||{},lab=src.label||ch;return {label:lab,data:years.map(function(y){return annualChannelValues(ch,focusName,focusIsSku,y,month);}),backgroundColor:ANNUAL_CH_COLORS[di%ANNUAL_CH_COLORS.length],hoverBackgroundColor:"#000000",borderColor:"#ffffff",borderWidth:1,borderRadius:di===ids.length-1?6:0,stack:"annual"};});var base=focus?(shipToLabel()+" · "+focusName):shipToLabel();setV("tgY",base+(month?" · "+MONTHS[month-1]:" · "+rangeLabel));charts["cAnnual"]=new Chart(document.getElementById("cAnnual"),{type:"bar",data:{labels:years,datasets:datasets},plugins:[annualLabelPlugin],options:{responsive:true,maintainAspectRatio:false,onClick:function(e,els){if(els&&els.length)pickKpi("year",years[els[0].index]);},plugins:{legend:{display:true,position:"bottom",labels:{usePointStyle:true,pointStyle:"rect",padding:14}},tooltip:{callbacks:{label:function(x){var total=0;x.chart.data.datasets.forEach(function(ds){total+=num(ds.data[x.dataIndex]);});var p=total?num(x.raw)/total*100:0;return x.dataset.label+": "+annualFmt(x.raw)+" MB ("+annualPct(p)+")";}}}},scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,beginAtZero:true,grace:"12%",ticks:{callback:function(v){return fmt(v);}}}}}});}
...
function drawCategory(c){var el=document.getElementById("cCat");if(!el||!Chart)return;destroy("cCat");var cT=document.getElementById("tgCat");if(cT)cT.textContent=(activeCat?("Category \u2022 "+activeCat):shipToLabel());
 ...
 charts["cCat"]=new Chart(el,{type:"doughnut",data:{labels:C2.map(function(x){return x.name;}),datasets:[{data:vals,backgroundColor:colors,borderWidth:selectedIndex>=0?0:1,borderColor:"#fff",_sd:_sd}]},plugins:[piePlugin],
  options:{responsive:true,maintainAspectRatio:false,cutout:"60%",onClick:function(ev,els){if(els&&els.length){var idx=els[0].index;if(C2[idx])toggleCat(C2[idx].name);}},
   plugins:{title:{display:false},pieText:{label:fmt(tot)+' MB',sub:""},legend:{display:false},tooltip:{callbacks:{label:function(x){var it=C2[x.dataIndex];return it.name+": "+fmt(it.mb)+" MB ("+num(it.pct).toFixed(1)+"%)";}}}}}});
 var lg="";C2.forEach(function(x,i){var colorIndex=selectedIndex>=0?selectedIndex:i;var bg=(activeCat===x.name)?"background:#fef9c3;":"";var shownPct=selectedIndex>=0?"100.0":num(x.pct).toFixed(1);lg+='<div class="catrow" data-cat="'+x.name+'" style="'+bg+'display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #f1f5f9;cursor:pointer;border-left:4px solid '+CATCOLR[colorIndex%CATCOLR.length]+';padding-left:8px"><span>'+x.name+'</span><b>'+fmt(x.mb)+' MB &middot; '+shownPct+'%</b></div>';});
 setT("catLegend",lg);
