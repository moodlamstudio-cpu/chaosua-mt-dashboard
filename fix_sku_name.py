...
d["top_sku"]=[{"name":v[1],"mb":round(v[0]/1e6,2),"cat":v[2]} for _,v in sorted(sku.items(),key=lambda x:-x[1][0])[:10]]
json.dump(d,open(dp,"w",encoding="utf-8"),ensure_ascii=False,indent=2)
print("Top 10 SKU (ใช้ description):")
for s in d["top_sku"]:
    print(f"  {s['mb']:6.2f} MB  [{s['cat']}]  {s['name']}")
wb.close()
